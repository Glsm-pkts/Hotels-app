...
import Avability from './Avability';
  ...
  return (
    ...
    <div className="border rounded-md p-4 gap-3 min-h-[300px] cursor-pointer grid grid-cols-6">
     ...
     <div className='flex items-center justify-between'>
       ...
       <Avability status={place.availability} />
       </div>
       <p>{place.location}</p>
       <div className="flex gap-4">{place.amenities.slice(0,2).map((i, key) =>(<span className="border p-1 rounded-md" key={key}>{i}</span>))}</div>
       <Rating point={place.rating} />
     </div>
     <div className="flex flex-col items-end">
      <span >{place.price_per_night}$</span>
